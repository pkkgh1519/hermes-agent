...
class SessionSource:
    ...
    route_mode: Optional[str] = None  # Route mode (e.g. notebooklm, notebooklm-hub)
...
def build_session_context_prompt(
    ...
    *,
    ...
    route_mode = (context.source.route_mode or "").strip().lower()
    if route_mode in {"notebooklm", "notebooklm-bound"}:
        lines.append(
    ...
    elif route_mode == "notebooklm-hub":
        lines.append(
            "**NotebookLM hub preference:** Prefer the `notebooklm` tool for notebook questions in this topic. "
            "This topic can use NotebookLM without a fixed bound notebook; list or select notebooks by name/number, "
            "then continue asking follow-up questions. Only use `terminal` for NotebookLM setup, debugging, or CLI fallback."
        )
