...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _is_group_chat(self, message: Message) -> bool:
        ...
        chat_type = str(raw_type).split(".")[-1].lower()
        if chat_type in {"group", "supergroup"}:
            return True
        # Test environments may replace python-telegram-bot enums with
        # MagicMock objects such as ``mock.ChatType.SUPERGROUP``.  Preserve the
        # production enum/string behavior above, but recognize those enum names
        # from their string/repr form so topic-route tests exercise the same
        # group path without requiring the optional Telegram package.
        raw_type_text = f"{raw_type!s} {raw_type!r}".lower()
        return "chattype.group" in raw_type_text or "chattype.supergroup" in raw_type_text
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        thread_id = getattr(message, "message_thread_id", None)
        allowed_topics = self._telegram_allowed_topics()
        if allowed_topics and not exact_route_matched:
            topic_id = str(thread_id) if thread_id is not None else self._GENERAL_TOPIC_THREAD_ID
            if topic_id not in allowed_topics:
                return False

        # Check legacy free/ignored thread lists for non-exact routes. Exact
        # topic routes carry their own ignored/free_response metadata above.
    ...
    def _build_message_event(
        ...
        raw_chat_type_text = f"{raw_chat_type!s} {raw_chat_type!r}".lower()
        chat_type = "dm"
        if telegram_chat_type in {"group", "supergroup"} or "chattype.group" in raw_chat_type_text or "chattype.supergroup" in raw_chat_type_text:
            chat_type = "group"
        elif telegram_chat_type == "channel" or "chattype.channel" in raw_chat_type_text:
            chat_type = "channel"
