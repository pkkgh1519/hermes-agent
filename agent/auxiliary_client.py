...
def _is_closed_client_error(exc: Exception) -> bool:
    """Detect httpx/OpenAI errors caused by reusing a closed cached client.

    A Codex auxiliary total-timeout intentionally closes the underlying
    transport to unblock the stream.  That timeout should stay a timeout, not
    be reclassified as a stale-cache bug just because its chained cause says
    the client was closed.
    """
    if isinstance(exc, TimeoutError) and str(exc).lower().startswith("codex auxiliary responses stream exceeded"):
        return False
    seen: set[int] = set()
    stack: list[Any] = [exc]
    while stack:
        ...
        if "client has been closed" in text:
            return True
        if "client is closed" in text and ("request" in text or "send" in text):
            return True
        stack.extend([
...
def _object_reports_closed(obj: Any) -> bool:
    """Best-effort closed-state probe that never performs I/O."""
    if obj is None:
        return False
    for attr in ("is_closed", "closed", "_closed"):
        try:
    ...
    try:
        ...
        if state is not None and str(state).lower().endswith("closed"):
            return True
    except Exception:
        pass
    return False
...
def _is_cached_client_closed(client: Any) -> bool:
    """Return True when a cached wrapper/client is known to be closed.

    Checks wrappers and their underlying OpenAI/httpx clients without sending a
    request.  This prevents long-lived gateway processes from reusing Codex
    auxiliary clients whose transport was closed to interrupt a timed-out
    Responses stream.
    """
    seen: set[int] = set()
...
def _close_cached_client(client: Any) -> None:
    """Close/mark a cached auxiliary client without awaiting async close paths."""
    import inspect
    ...
    for attr in ("_real_client", "_client"):
        try:
            child = getattr(client, attr, None)
        except Exception:
            child = None
        if child is not None and child not in candidates:
            candidates.append(child)
    for candidate in candidates:
        _force_close_async_httpx(candidate)
        try:
            close_fn = getattr(candidate, "close", None)
        except Exception:
            continue
        if not callable(close_fn) or inspect.iscoroutinefunction(close_fn):
            continue
        try:
            close_fn()
        except Exception:
...
def _get_cached_client(
    ...
    with _client_cache_lock:
        if cache_key in _client_cache:
            ...
            if _is_cached_client_closed(cached_client):
                logger.info(
                    "Auxiliary client cache hit for %s is closed; rebuilding",
                    provider or "auto",
                )
...
def call_llm(
    ...
    *,
    ...
    active_cache_provider = resolved_provider
    active_cache_model = resolved_model
    active_cache_base_url = resolved_base_url
    active_cache_api_key = resolved_api_key
    active_cache_api_mode = resolved_api_mode
    ...
    if task == "vision":
        ...
        if client is None:
            ...
            if not resolved_base_url:
                ...
                active_cache_provider = "auto"
                active_cache_model = None
                active_cache_base_url = None
                active_cache_api_key = None
                active_cache_api_mode = None
        if client is None:
            raise RuntimeError(
                f"No LLM provider configured for task={task} provider={resolved_provider}. "
                f"Run: hermes setup")
    ...
    if task:
        logger.info("Auxiliary %s: using %s (%s)%s",
                     task, active_cache_provider or resolved_provider or "auto", final_model or "default",
    ...
    kwargs = _build_call_kwargs(
        active_cache_provider or resolved_provider, final_model, messages,
    ...
    if _is_anthropic_compat_endpoint(active_cache_provider or resolved_provider, _client_base):
    ...
    try:
        ...
        if _is_closed_client_error(first_err) and task != "vision":
            retry_provider = active_cache_provider or resolved_provider or "auto"
            logger.info(
                "Auxiliary %s: cached client was closed for %s; rebuilding and retrying once",
                task or "call",
                retry_provider,
            )
            try:
                ...
                logger.debug("Auxiliary: closed-client cache eviction failed", exc_info=True)
            try:
                fresh_client, fresh_model = _get_cached_client(
                    retry_provider,
                    active_cache_model,
                    base_url=active_cache_base_url,
                    api_key=active_cache_api_key,
                    api_mode=active_cache_api_mode,
                    main_runtime=main_runtime,
                )
                if fresh_client is not None:
                    fresh_base = str(getattr(fresh_client, "base_url", active_cache_base_url) or "")
                    fresh_kwargs = _build_call_kwargs(
                        retry_provider,
                        fresh_model,
                        messages,
                        temperature=temperature,
                        ...
                        timeout=effective_timeout,
                        extra_body=effective_extra_body,
                        base_url=fresh_base or active_cache_base_url,
                    )
                    if _is_anthropic_compat_endpoint(retry_provider, fresh_base):
                        fresh_kwargs["messages"] = _convert_openai_images_to_anthropic(fresh_kwargs["messages"])
                    try:
                        return _validate_llm_response(
                            fresh_client.chat.completions.create(**fresh_kwargs), task)
                    except Exception as retry_err:
                        first_err = retry_err
                        client = fresh_client
                        final_model = fresh_model
                        kwargs = fresh_kwargs
                        _base_info = fresh_base
                        if retry_provider == "auto" or resolved_provider not in {"auto", "", None}:
                            resolved_provider = retry_provider
            except Exception as rebuild_err:
                first_err = rebuild_err
