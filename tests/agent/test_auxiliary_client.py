...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    def test_codex_timeout_evicts_cached_wrapper(self):
        ...
        class FakeClient:
            def __init__(self):
                self.responses = SimpleNamespace(create=lambda **k: SlowAliveCreateStream())
                self.api_key = "k"
    ...
    def test_closed_client_detector_is_narrow(self):
        from agent.auxiliary_client import _is_closed_client_error

        assert _is_closed_client_error(
            RuntimeError("Cannot send a request, as the client has been closed.")
        ) is True
        timeout = TimeoutError("Codex auxiliary Responses stream exceeded 120.0s total timeout")
        timeout.__cause__ = RuntimeError("Cannot send a request, as the client has been closed.")
        assert _is_closed_client_error(timeout) is False

    def test_get_cached_client_rebuilds_closed_cache_hit(self):
        from agent.auxiliary_client import (
            _client_cache,
            _client_cache_key,
            _client_cache_lock,
        ...
        closed_client = SimpleNamespace(is_closed=lambda: True, close=lambda: None)
        fresh_client = SimpleNamespace(base_url="https://chatgpt.com/backend-api/codex")
        cache_key = _client_cache_key("openai-codex", async_mode=False)
        with _client_cache_lock:
            _client_cache.clear()
            _client_cache[cache_key] = (closed_client, "old-model", None)
        try:
            with patch(
                "agent.auxiliary_client.resolve_provider_client",
                return_value=(fresh_client, "fresh-model"),
            ) as resolver:
                client, model = _get_cached_client("openai-codex", "requested-model")
    ...
    def test_call_llm_retries_closed_auto_fallback_client_once(self):
        poisoned = MagicMock(name="poisoned_auto_client")
        poisoned.base_url = "https://chatgpt.com/backend-api/codex"
        poisoned.chat.completions.create.side_effect = RuntimeError(
            "Cannot send a request, as the client has been closed."
        )
        fresh = MagicMock(name="fresh_auto_client")
        fresh.base_url = "https://chatgpt.com/backend-api/codex"
        fresh.chat.completions.create.return_value = SimpleNamespace(
        ...
        def fake_get_cached(provider, *args, **kwargs):
            ...
            if provider == "custom":
                return None, None
            if calls.count("auto") == 1:
                return poisoned, "gpt-5.5"
            return fresh, "gpt-5.5"
        ...
        assert calls == ["custom", "auto", "auto"]
        poisoned.chat.completions.create.assert_called_once()
        fresh.chat.completions.create.assert_called_once()
